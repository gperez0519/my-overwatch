...
const LaunchRequestHandler = {
  ...
  async handle(handlerInput) {
    ...
    let GREETING_PERSONALIZED_II =
      VocalResponses.responses.GREETING_PERSONALIZED_II;
    let GREETING_PERSONALIZED_III =
      VocalResponses.responses.GREETING_PERSONALIZED_III;
    ...
    let GREETING_PERSONALIZED_DISPLAY =
      VocalResponses.responses.GREETING_PERSONALIZED_DISPLAY;
    let GREETING_PERSONALIZED_II_DISPLAY =
      VocalResponses.responses.GREETING_PERSONALIZED_II_DISPLAY;
    let GREETING_PERSONALIZED_III_DISPLAY =
      VocalResponses.responses.GREETING_PERSONALIZED_III_DISPLAY;
...
const GetMyStatsIntentHandler = {
  ...
  async handle(handlerInput) {
    ...
    let rePromptText = `Well, I guess you don't find that to be very exciting but in any case I will leave you to it. Let me know if you need anything else. ${
      drinkCount > 2
        ? VocalResponses.responses.TOO_MANY_DRINKS_OPTIONS
        : VocalResponses.responses.OPTIONS
    }`;
    ...
    if (battletag_username && battletag_number) {
      ...
      try {
        ...
        if (isObjectEmpty(stats)) {
          outputSpeech =
            " " + VocalResponses.responses.OVERWATCH_STATS_NOT_AVAILABLE;
          ...
          if (!isObjectEmpty(mostPlayed)) {
            ...
            if (!isObjectEmpty(stats.rank)) {
              ...
              if (!isObjectEmpty(stats.heroStats.competitive)) {
                ...
                var suggestedCompHero = getBestHeroForComp(
                  mostPlayedCompetitiveHero,
...
const OverwatchLeagueTeamInfoIntentIntentHandler = {
  ...
  async handle(handlerInput) {
    ...
    try {
      ...
      return handlerInput.responseBuilder
        .speak(
          `<voice name='Emma'>${VocalResponses.responses.OVERWATCH_LEAGUE_TEAM_SERVICE_UNAVAILABLE}</voice>`
        )
        .reprompt(
          `<voice name='Emma'>${VocalResponses.responses.OVERWATCH_LEAGUE_TEAM_SERVICE_UNAVAILABLE}</voice>`
        )
    ...
    return handlerInput.responseBuilder
      .speak(
        `<voice name='Emma'>${outputSpeech} ${VocalResponses.responses.ANYTHING_ELSE} ${options}</voice>`
      )
      .reprompt(
        `<voice name='Emma'>${VocalResponses.responses.PLEASE_REPEAT} ${VocalResponses.responses.ANYTHING_ELSE} ${options}</voice>`
      )
      .withStandardCard(
...
const OverwatchLeagueUpcomingMatchesIntentHandler = {
  ...
  async handle(handlerInput) {
          ...
          for (const match in matches) {
            if (Object.hasOwnProperty.call(matches, match)) {
              ...
              if (element.startDate) {
                var eventStartDate = momentTZ
                  .utc(element.startDate)
                  .tz(userTimeZone);
                var curDateTime = momentTZ.utc().tz(userTimeZone);
                var eventStartDateOnly = momentTZ
                  .utc(element.startDate)
                  .tz("America/New_York")
                  .format("MM-DD-YYYY");
                var yesterdayDateOnly = momentTZ
                  .utc()
                  .tz("America/New_York")
                  .subtract(1, "days")
                  .format("MM-DD-YYYY");
                var curDateOnly = momentTZ
                  .utc()
                  .tz("America/New_York")
                  .format("MM-DD-YYYY");
                ...
                var minutesPastEvent = curDateTime.diff(
                  eventStartDate,
                  "minutes"
                );
    ...
    outputSpeech =
      VocalResponses.responses.OVERWATCH_LEAGUE_SERVICE_UNAVAILABLE;
    ...
    await x("https://overwatchleague.com/en-us/schedule/", "#__NEXT_DATA__")
      ...
      .then((overwatchLeagueData) => {
        var overwatchLeagueScheduleBlock =
          overwatchLeagueData.props.pageProps.blocks[2].schedule;
        var overwatchLeagueScheduleMatches =
...
const RandomRoleHeroGeneratorIntentHandler = {
  ...
  handle(handlerInput) {
    ...
    return handlerInput.responseBuilder
      .speak(
        `<voice name='Emma'>${outputSpeech} ${VocalResponses.responses.HERO_MORE_INFO_PROMPT}</voice>`
      )
      .reprompt(
        `<voice name='Emma'>${VocalResponses.responses.PLEASE_REPEAT} ${VocalResponses.responses.HERO_MORE_INFO_PROMPT}</voice>`
      )
      .withStandardCard(
        sessionAttributes["hero-name"]
...
const SomethingElseOptionsIntentHandler = {
  ...
  handle(handlerInput) {
    ...
    return handlerInput.responseBuilder
      .speak(`<voice name='Emma'>${outputSpeech}</voice>`)
      .reprompt(
        `<voice name='Emma'>${VocalResponses.responses.PLEASE_REPEAT} ${options}</voice>`
      )
...
const AnotherDrinkIntentHandler = {
  ...
  handle(handlerInput) {
    ...
    return handlerInput.responseBuilder
      .speak(`<voice name='Emma'>${speechText}</voice>`)
      .reprompt(
        `<voice name='Emma'>${VocalResponses.responses.PLEASE_REPEAT} ${speechText}</voice>`
      )
...
const HelpIntentHandler = {
  ...
  handle(handlerInput) {
    ...
    return handlerInput.responseBuilder
      .speak(`<voice name='Emma'>${outputSpeech}</voice>`)
      .reprompt(
        `<voice name='Emma'>${VocalResponses.responses.HELP_REPEAT_PROMPT} ${options}</voice>`
      )
...
const NoIntentHandler = {
  ...
  handle(handlerInput) {
    ...
    return handlerInput.responseBuilder
      .speak(
        `<voice name='Emma'>Very well, ${VocalResponses.responses.ANYTHING_ELSE} ${options}</voice>`
      )
      .reprompt(
        `<voice name='Emma'>${VocalResponses.responses.PLEASE_REPEAT} ${options}</voice>`
      )
...
const ErrorHandler = {
  ...
  handle(handlerInput, error) {
    ...
    return handlerInput.responseBuilder
      .speak(
        `<voice name='Emma'>${VocalResponses.responses.ERROR_PROMPT}, ${VocalResponses.responses.ANYTHING_ELSE} ${options}</voice>`
      )
      .reprompt(
        `<voice name='Emma'>${VocalResponses.responses.ERROR_PROMPT}, ${VocalResponses.responses.ANYTHING_ELSE} ${options}</voice>`
      )
...
const FallbackIntentHandler = {
  ...
  handle(handlerInput) {
    ...
    return handlerInput.responseBuilder
      .speak(
        `<voice name='Emma'>${VocalResponses.responses.FALLBACK_PROMPT} ${VocalResponses.responses.ANYTHING_ELSE} ${options}</voice>`
      )
      .reprompt(
        `<voice name='Emma'>${VocalResponses.responses.FALLBACK_PROMPT} ${VocalResponses.responses.ANYTHING_ELSE} ${options}</voice>`
      )
